...
% Aconitase (ACO)
V_ACO = k*k_f_ACO.*(C_CIT - (C_ISOC./k_E_ACO));
...
V_IDH = k*k_cat_IDH.*E_T_IDH.*(1 + (C_H./k_h_1) + (k_h_2./C_H) + f_i_IDH.*(K_M_NAD./C_NAD) + f_a_IDH.*(K_M_ISOC./C_ISOC).^Ni + f_a_IDH.*f_i_IDH.*(K_M_ISOC./C_ISOC).^Ni.*(K_M_NAD./C_NAD)).^-1;

dC_ISOC = V_ACO - V_IDH;

% Alpha-ketoglutarate dehydrogenase (KGDH)
f_a_KGDH = ((1 + (C_Mg./K_D_Mg)).*(1 + (Ca_m./K_D_Ca))).^(-1);
V_KGDH = k*(k_cat_KGDH.*E_T_KGDH)./(1 + f_a_KGDH.*(K_M_aKG./C_aKG).^(n_aKG)+f_a_KGDH.*((K_M_NAD_new)./C_NAD)); % ?????? why not K_M_NAD??

dC_aKG = V_IDH - V_KGDH + V_AAT;

% Succinyl CoA lyase (SL)
V_SL = k*k_f_SL.*((C_SCoA.*C_ADP_m - (C_Suc.*C_ATP_m.*C_CoA)./(k_E_SL)));
dC_SCoA = V_KGDH - V_SL;

% Succinate dehydrogenase (SDH)
V_SDH = k*(k_cat_SDH.*E_T_SDH)./...
    (1 + (K_M_SUC./C_Suc).*(1 + C_OAA./K_i_sdh_OAA).*(1 + C_FUM./K_i_FUM));

dC_Suc = V_SL - V_SDH;

% Fumarate hydratase (FH)
V_FH = k*k_f_FH.*(C_FUM - C_MAL./(K_E_FH));
dC_FUM = V_SDH - V_FH;

% Malate dehydrogenase (MDH)
f_h_a = (1 + C_H./k_h1 + C_H.^2./(k_h1.*k_h2)).^-1 + k_offset;
f_h_i = (1 + k_h3./C_H + (k_h3.*k_h4)./C_H.^2).^-2;
V_MDH = k*(k_cat_MDH.*E_T_MDH.*f_h_a.*f_h_i)./(1 + (K_M_MAL./C_MAL).*(1 + (C_OAA./K_i_OAA)) + (K_M_NAD_mdh./C_NAD) + ((K_M_MAL./C_MAL).*(1 + (C_OAA./K_i_OAA)).*(K_M_NAD_mdh))./(C_NAD)); % ???????? Why different K_M_NAD_mdh????
