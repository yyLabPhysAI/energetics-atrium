...
g_Kto = 50.02;
I_Kto = 0.35*g_Kto*r*(0.59*s_1^3 + 0.41*s_2^3)*(0.6*s_3^6 + 0.4)*(V - E_k); % CT - 0.2, PM - 0.35

g_sus = 2.4; % ???????? 
E_sus = 70; % ??????
Isus = g_sus*(E_sus + 70);  %CT - 1.4, PM - 2.4
I_Kto  = I_Kto + Isus;
...
tau_s_1 = 0.5466/(1 + exp((V + 32.8)/0.1)) + 0.0204; % Lindblad ?????? Do we need Aslandi too?
ds_1 = (s_1m - s_1)/tau_s_1;

s_2m = 1/(1 + exp((V + 28.29)/7.06));
tau_s_2 = 5.75/(1 + exp((V + 32.8)/0.1)) + 0.45/(1 + exp(-(V - 13.54)/13.97)); % Aslandi ??????? discrepency with s1?
