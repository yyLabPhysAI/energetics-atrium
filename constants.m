...
data.C_A_i      = 7.99;             % [mM]          Total sum of cytoplasmatic adenine nucleotides
...
data.r_c1		= 2.656e-22; 	    % 1/[ms] 		Sum of products of rate constants ??? Can we replace this expression with it's result????
data.r_c2		= 8.632e-30;		% 1/[ms] 		Sum of products of rate constants
data.r_1		= 2.077e-18;       % [] 	Sum of products of rate constants ??? Can we replace this expression with it's result????
data.r_2		= 1.728e-9;		 	% []			Sum of products of rate constants 
data.r_3		= 1.059e-26;		% []			Sum of products of rate constants
data.rho_res	= 0.05; 		% [mM] 			Concentration of electron carriers (respiratory complexes I-III-IV) ??? Can we replace this expression with it's result????
data.K_res		= 1.35e18;			% []   			Equilibrium constant of respiration
data.rho_resF	= 0.0045; 			% [mM] 			Concentration of electron carriers (respiratory complexes II-III-IV)
data.psi_B		= 50; 			% [mV] 			Phase boundary potential
...
data.p_c1		= 9.6510e-17;	    % 1/[ms]		Sum of products of rate constants	??? Can we replace this expression with it's result????
data.p_c2		= 7.7390e-17;	    % 1/[ms]		Sum of products of rate constants	??? Can we replace this expression with it's result????
...
data.Pi			= 20; 				% [mM] 			Inorganic phosphate concentration
data.C_A_m		= 1.5; 				% [mM] 			Total sum of mitochondrial adenine nucleotides
data.V_ant_max	= 0.025;		% [mM/ms] 		Maximal rate of ANT
data.h_ANT		= 0.5; 			% [] 			Fraction of delta_psi_m
...
data.k_cat_cs	= 0.0032;				% [1/ms] 		Catalytic constant of CS
data.E_T_cs		= 0.4;				% [mM]			Concentration of CS
data.K_M_AcCoA	= 1.26e-2; 			% [mM]			Michaelis constant of AcCoA
data.K_M_OAA	= 6.4e-4;			% [mM]			Michaelis constant of OAA
data.C_K_int	= 1; 				% [mM]			Sum of TCA cycle intermediates' concentration
data.k_f_ACO	= 12.5e-3; 			% 1/[ms]		Forward rate constant of ACO
data.k_E_ACO	= 2.22;   			% [] 			Equilibrium constant of ACO
data.K_ADP_a	= 6.2e-2;				% [mM]			Activation constant by ADP
data.K_Ca_a		= 0.0005;			% [mM]			Activation constant by ADP
data.K_i_NADH	= 0.19;				% [mM]			Inhibition constant by NADH
data.k_cat_IDH	= 0.003;				% 1/[ms]		Rate constant of IDH
...
data.K_M_NAD	= 38.7;			% [mM]			Michaelis constant for NAD+
...
data.k_cat_KGDH	= 0.005; 			% 1/[ms]		Rate constant of KGDH
data.K_M_aKG	= 1.94;			% [mM]			Michaelis constant for aKG
data.K_M_NAD_new= 38.7;			% [mM]			Michaelis constant for NAD (no plus)
data.n_aKG		= 1.2; 			% [] 			Hill coefficient of KGDH for aKG
data.C_Mg		= 0.4;  			% [mM]			Mg2+ concentration in mitochondria
data.k_f_SL		= 0.127; 				% [mM]/[ms]		Forward rate constant of SL
data.k_E_SL		= 3.115;  			% [] 			Equilibrium constant of the SL reaction
data.C_CoA		= 2e-02; 			% [mM]			Coenzyme A cocentration
data.k_cat_SDH	= 1e-3;  			% 1/[ms]		Rate constant of SDH
...
data.k_f_FH		= 0.83e-3; 			% 1/[ms]		Forward rate constant of FH
data.K_E_FH		= 1;  			% [] 			Equilibrium constant of FH
...
data.k_cat_MDH	= 2.775e-2;  			% 1/[ms] 		Rate constant of MDH
...
data.K_E_AAT	= 6.6; 			% []			Eqilibrium constant of AAT
data.k_ASP		= 0.01e-3; 				% 1/[ms]		Rate constant of aspartate consumption
